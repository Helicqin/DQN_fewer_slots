...
class NLU:
    def __init__(self, config_dir="./NLU/data/words_labels.json", dictionary_dir="./NLU/data/dic.txt",
                 slot_semantic_dir="./NLU/data/slot_semantic_dict.json", stopwords_dir="./NLU/data/stopword.txt"):
        jieba.load_userdict(dictionary_dir)
        with open(slot_semantic_dir, 'r', encoding='utf-8') as f:
            self.slot_dict = json.load(f)
        # elman network
        self.model = RNN(nh=100, nc=127, ne=572, de=100, cs=7)  # 导入一个训练好的模型
        with open(config_dir, 'r', encoding='utf-8') as f:
            value = json.load(f)
            self.word_to_index = value['words2idx']
            self.label_to_index = value["labels2idx"]
            self.index_to_word = dict((k, v) for v, k in value['words2idx'].items())
            self.index_to_label = dict((k, v) for v, k in value['labels2idx'].items())
        # 读取stopwords
        self.stopwords = []
    ...
    def words_to_index(self, raw_sentence):
        """将raw_sentence转为index list,同时返回index 和 words_list"""
        words_list, _ = self.participle(raw_sentence)
        return [self.word_to_index[i] for i in words_list], words_list

    def elman_get_iob(self, words_index):
        """input a words index list of numpy array,output a list of iob label"""
        pre_y = self.model.classify(contextwin(words_index, 7))   # index
        return [self.index_to_label[i] for i in pre_y]

    def elman_get_diaact(self, raw_sentence, history):
        """elman nlu"""
        words_index, words_list = self.words_to_index(raw_sentence)
        print("words: ", words_list)
        iob = self.elman_get_iob(words_index)
        print("iob: ", iob)
        diaact = self.iob_to_diaact(iob, string=words_list, history=history, raw_sentence=raw_sentence)
        return diaact
# nlu = NLU()
# s = '2:30'
