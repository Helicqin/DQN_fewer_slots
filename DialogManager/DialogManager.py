'''
The code is just for generating simulated dialogues, not for real DM.And you must use agent_response_with_diaact not agent_response
Modified :
    self.agent_request_slots = ["child_age", "client_location", "reserve_location", "phone_number",
                                "reserve_time", "client_name", "client_gender"]
'''
...
class DM:
    ...
    def agent_response(self, raw_usr_sentence):
        ...
        usr_diaact = self.nlu.elman_get_diaact(raw_usr_sentence, history=self.history)
                    ...
                    if usr_current_request_slot != []:  # 默认user只能问一个问题
                        ...
                        if "client_location" in usr_diaact["inform_slots"]:
                            ...
                            agent_diaact = {"diaact": "select", "request_slots": {'reserve_location': 'UNK'},
                                            "inform_slots": {'reserve_location': json.dumps(self.location[city][town])}}
    ...
    def agent_response_with_diaact(self, usr_diaact):
        """针对于用户的每句话，agent作出的回复，返回diaact，仅用于生成模拟对话"""
        # print("usr_diaact:", usr_diaact)
        # 更新history
        dic = {
            "speaker": "user",
        ...
        if self.turn == 0:  # 第一次需要判断用户目的：咨询还是加盟
            ...
            if self.last_request_slot in self.agent_inform_slots:
                        ...
                        if "client_location" in usr_diaact["inform_slots"]:
                            ...
                            if 'child_age' in self.agent_inform_slots:  # 判断年龄
                                ...
                                if 3 <= age <= 5:
                                    ...
                                    self.index = 3  # no matching,close the dialog
                                    self.flag = True
                                agent_diaact = {"diaact": "request",
                                                "request_slots": {self.agent_request_slots[0]: "UNK"},
                                                "inform_slots": {"special_need": "UNK"}}
                                self.last_request_slot = self.agent_request_slots[0]
                                del self.agent_request_slots[0]
                            else:
                ...
                if self.last_request_slot == "phone_number":
                    self.agent_request_slots.append("phone_number")
                    # 判断用户是否在提问
                    if usr_current_request_slot != []:  # 默认user只能问一个问题
                        agent_diaact = {"diaact": "request", "request_slots": {self.agent_request_slots[0]: "UNK"},
                                        "inform_slots": {usr_current_request_slot[0]: "UNK"}}
